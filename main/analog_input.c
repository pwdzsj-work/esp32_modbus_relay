...
#define ANALOG_LOG_PERIOD_MS 1000
...
void analog_input_sample_all(void)
{
    for (int i = 0; i < BOARD_ANALOG_COUNT; ++i) {
        /*
         * Discard the first conversion after changing the ADS1115 MUX.
         * This gives high-impedance input networks time to settle and avoids
         * carrying charge from the previously selected channel.
         */
        int16_t discard;
        esp_err_t err = ads1115_read_raw(s_ads_channel[i], &discard);
        if (err != ESP_OK) {
            ESP_LOGW(TAG, "AN%d/AIN%d settling sample failed: %s",
                     i + 1, s_ads_channel[i], esp_err_to_name(err));
            continue;
        }

        int16_t raw;
        err = ads1115_read_raw(s_ads_channel[i], &raw);
        if (err == ESP_OK) {
            ...
            ESP_LOGW(TAG, "AN%d/AIN%d sample failed: %s",
                     i + 1, s_ads_channel[i], esp_err_to_name(err));
        }
    }

    TickType_t now = xTaskGetTickCount();
    if (now - s_last_log >= pdMS_TO_TICKS(ANALOG_LOG_PERIOD_MS)) {
        xSemaphoreTake(s_lock, portMAX_DELAY);
        for (int i = 0; i < BOARD_ANALOG_COUNT; ++i) {
            ESP_LOGI(TAG,
                     "AN%d<-AIN%d raw=%d, adc=%.3fV, terminal=%.3fV",
                     i + 1, s_ads_channel[i], s_raw[i],
                     (double)s_adc_v[i], (double)(s_adc_v[i] * 3.7f));
        }
        xSemaphoreGive(s_lock);
        s_last_log = now;
    }
